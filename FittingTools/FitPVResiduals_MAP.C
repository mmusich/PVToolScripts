...
void arrangeCanvas2D(TCanvas *canv,TH2F* meanmaps[100],TH2F* widthmaps[100],Int_t nFiles,TString LegLabels[10],TString theLeg="bogus");
void arrangeFitCanvas(TCanvas *canv,TH1F* meanplots[100],Int_t nFiles, TString LegLabels[10],TString theLeg="bogus");
...
//*************************************************************
  ...
  for(Int_t i=0;i<nFiles_;i++){
    ...
    // 2D maps
    dxyMeanMap[i]      = new TH2F(Form("means_dxy_map_%i",i), "#LT d_{xy} #GT map;track #eta;track #varphi [rad];#LT d_{xy} #GT [#mum]",nBins_,lowedge,highedge,nBins_,lowedge,highedge);		       
    dzMeanMap[i]       = new TH2F(Form("means_dz_map_%i",i),"#LT d_{z} #GT map;track #eta;track #varphi [rad];#LT d_{z} #GT [#mum]",nBins_,lowedge,highedge,nBins_,lowedge,highedge);  		       
    dxyNormMeanMap[i]  = new TH2F(Form("norm_means_dxy_map_%i",i),"#LT d_{xy}/#sigma_{d_{xy}} #GT map;track #eta;track #varphi [rad];#LT d_{xy}/#sigma_{d_{xy}} #GT",nBins_,lowedge,highedge,nBins_,lowedge,highedge);  
    dzNormMeanMap[i]   = new TH2F(Form("norm_means_dz_map_%i",i),"#LT d_{z}/#sigma_{d_{z}} #GT map;track #eta;track #varphi[rad];#LT d_{xy}/#sigma_{d_{z}} #GT",nBins_,lowedge,highedge,nBins_,lowedge,highedge);    
 
    dxyWidthMap[i]     = new TH2F(Form("widths_dxy_map_%i",i),"#sigma_{d_{xy}} map;track #eta;track #varphi [rad];#sigma(d_{xy}) [#mum]",nBins_,lowedge,highedge,nBins_,lowedge,highedge);		       
    dzWidthMap[i]      = new TH2F(Form("widths_dz_map_%i",i),"#sigma_{d_{z}} map;track #eta;track #varphi [rad];#sigma(d_{z}) [#mum]",nBins_,lowedge,highedge,nBins_,lowedge,highedge);  		       
    dxyNormWidthMap[i] = new TH2F(Form("norm_widths_dxy_map_%i",i),"width(d_{xy}/#sigma_{d_{xy}}) map;track #eta;track #varphi[rad];#sigma(d_{xy}/#sigma_{d_{xy}})",nBins_,lowedge,highedge,nBins_,lowedge,highedge);  
    dzNormWidthMap[i]  = new TH2F(Form("norm_widths_dz_map_%i",i),"width(d_{z}/#sigma_{d_{z}}) map;track #eta;track #varphi [rad];#sigma(d_{z}/#sigma_{d_{z}})",nBins_,lowedge,highedge,nBins_,lowedge,highedge); 
...
//*************************************************************
{
  ...
  for(Int_t i=0; i<nFiles; i++){
    pt[i] = new TPaveText(0.13,0.95,0.191,0.98,"NDC");
