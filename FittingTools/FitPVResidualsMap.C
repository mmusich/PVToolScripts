...
void arrangeCanvas2D(TCanvas *canv,TH2F* meanmaps[100],TH2F* widthmaps[100],Int_t nFiles,TString LegLabels[10],TString theLeg);
...
//*************************************************************
  ...
  for(Int_t i=0;i<nFiles_;i++){
	...
	// absolute residuals
	//std::cout<<Form("PVValidation/Abs_DoubleDiffResiduals/histo_dxy_eta_plot%i_phi_plot%i",j,k)<<std::endl;   
	dxyMapResiduals[i][j][k] = (TH1F*)fins[i]->Get(Form("PVValidation/Abs_DoubleDiffResiduals/histo_dxy_eta_plot%i_phi_plot%i",j,k));
	//std::cout<<Form("PVValidation/Abs_DoubleDiffResiduals/histo_dxy_eta_plot%i_phi_plot%i",j,k)<<std::endl;   
	dzMapResiduals[i][j][k]  = (TH1F*)fins[i]->Get(Form("PVValidation/Abs_DoubleDiffResiduals/histo_dxy_eta_plot%i_phi_plot%i",j,k));  

	// normalized residuals
	//std::cout<<Form("PVValidation/Norm_DoubleDiffResiduals/histo_dxy_eta_plot%i_phi_plot%i",j,k)<<std::endl;  
	...
	//std::cout<<Form("PVValidation/Norm_DoubleDiffResiduals/histo_dxy_eta_plot%i_phi_plot%i",j,k)<<std::endl;  
	dzNormMapResiduals[i][j][k]  = (TH1F*)fins[i]->Get(Form("PVValidation/Norm_DoubleDiffResiduals/histo_norm_dxy_eta_plot%i_phi_plot%i",j,k));
  ...
  for(Int_t i=0;i<nFiles_;i++){
    ...
    dxyNormPhiMeanTrend[i] = new TH1F(Form("means_dxyNorm_phi_%i",i),"#LT d_{xy}/#sigma_{d_{xy}} #GT vs #phi sector;#varphi (sector) [degrees];#LT d_{xy}/#sigma_{d_{xy}} #GT",nBins_,lowedge,highedge); 
    dxyNormPhiWidthTrend[i]= new TH1F(Form("widths_dxyNorm_phi_%i",i),"#sigma(d_{xy}/#sigma_{d_{xy}}) vs #phi sector;#varphi (sector) [degrees];#sigma(d_{xy}/#sigma_{d_{xy}}) ",nBins_,lowedge,highedge);
    dzNormPhiMeanTrend[i]  = new TH1F(Form("means_dzNorm_phi_%i",i),"#LT d_{z}/#sigma_{d_{z}} #GT vs #phi sector;#varphi (sector) [degrees];#LT d_{z}/#sigma_{d_{z}} #GT",nBins_,lowedge,highedge); 
    dzNormPhiWidthTrend[i] = new TH1F(Form("widths_dzNorm_phi_%i",i),"#sigma(d_{z}/#sigma_{d_{z}}) vs #phi sector;#varphi (sector) [degrees];#sigma(d_{z}/#sigma_{d_{z}})",nBins_,lowedge,highedge);
    
    dxyNormEtaMeanTrend[i] = new TH1F(Form("means_dxyNorm_eta_%i",i),"#LT d_{xy}/#sigma_{d_{xy}} #GT vs #eta sector;#eta (sector);#LT d_{xy}/#sigma_{d_{xy}} #GT",nBins_,lowedge,highedge);
    dxyNormEtaWidthTrend[i]= new TH1F(Form("widths_dxyNorm_eta_%i",i),"#sigma(d_{xy}/#sigma_{d_{xy}}) vs #eta sector;#eta (sector);#sigma(d_{xy}/#sigma_{d_{xy}})",nBins_,lowedge,highedge);
    dzNormEtaMeanTrend[i]  = new TH1F(Form("means_dzNorm_eta_%i",i),"#LT d_{z}/#sigma_{d_{z}} #GT vs #eta sector;#eta (sector);#LT d_{z}/#sigma_{d_{z}} #GT",nBins_,lowedge,highedge); 
    dzNormEtaWidthTrend[i] = new TH1F(Form("widths_dzNorm_eta_%i",i),"#sigma(d_{z}/#sigma_{d_{z}}) vs #eta sector;#eta (sector);#sigma(d_{z}/#sigma_{d_{z}})",nBins_,lowedge,highedge);

    // 2D maps

    dxyMeanMap[i]      = new TH2F(Form("means_dxy_map_%i",i), "#LT d_{xy} #GT map;#eta (sector);#varphi (sector) [degrees];#LT d_{xy} #GT [#mum]",nBins_,lowedge,highedge,nBins_,lowedge,highedge);		       
    dzMeanMap[i]       = new TH2F(Form("means_dz_map_%i",i),"#LT d_{z} #GT map;#eta (sector);#varphi (sector) [degrees];#LT d_{z} #GT [#mum]",nBins_,lowedge,highedge,nBins_,lowedge,highedge);  		       
    dxyNormMeanMap[i]  = new TH2F(Form("norm_means_dxy_map_%i",i),"#LT d_{xy}/#sigma_{d_{xy}} #GT map;#eta (sector);#varphi (sector) [degrees];#LT d_{xy}/#sigma_{d_{xy}} #GT",nBins_,lowedge,highedge,nBins_,lowedge,highedge);  
    dzNormMeanMap[i]   = new TH2F(Form("norm_means_dz_map_%i",i),"#LT d_{z}/#sigma_{d_{z}} #GT map;#eta (sector);#varphi (sector) [degrees];#LT d_{xy}/#sigma_{d_{z}} #GT",nBins_,lowedge,highedge,nBins_,lowedge,highedge);    
 
    dxyWidthMap[i]     = new TH2F(Form("widths_dxy_map_%i",i),"#sigma_{d_{xy}} map;#eta (sector);#varphi (sector) [degrees];#sigma(d_{xy}) [#mum]",nBins_,lowedge,highedge,nBins_,lowedge,highedge);		       
    dzWidthMap[i]      = new TH2F(Form("widths_dz_map_%i",i),"#sigma_{d_{z}} map;#eta (sector);#varphi (sector) [degrees];#sigma(d_{z}) [#mum]",nBins_,lowedge,highedge,nBins_,lowedge,highedge);  		       
    dxyNormWidthMap[i] = new TH2F(Form("norm_widths_dxy_map_%i",i),"width(d_{xy}/#sigma_{d_{xy}}) map;#eta (sector);#varphi (sector) [degrees];#sigma(d_{xy}/#sigma_{d_{xy}})",nBins_,lowedge,highedge,nBins_,lowedge,highedge);  
    dzNormWidthMap[i]  = new TH2F(Form("norm_widths_dz_map_%i",i),"width(d_{z}/#sigma_{d_{z}}) map;#eta (sector);#varphi (sector) [degrees];#sigma(d_{z}/#sigma_{d_{z}})",nBins_,lowedge,highedge,nBins_,lowedge,highedge);     
  ...
  dzPhiTrendFit->SaveAs("dzPhiTrendFit.pdf");
  dzPhiTrendFit->SaveAs("dzPhiTrendFit.png");
  ...
  dxyNormPhiTrend->SaveAs("dxyNormPhiTrend.pdf");
  dxyNormPhiTrend->SaveAs("dxyNormPhiTrend.png");
  ...
  dzNormPhiTrend->SaveAs("dzNormPhiTrend.pdf");
  dzNormPhiTrend->SaveAs("dzNormPhiTrend.png");
  ...
  dxyNormEtaTrend->SaveAs("dxyNormEtaTrend.pdf");
  dxyNormEtaTrend->SaveAs("dxyNormEtaTrend.png");
  ...
  dzNormEtaTrend->SaveAs("dzNormEtaTrend.pdf");
  dzNormEtaTrend->SaveAs("dzNormEtaTrend.png");

  // 2D Maps
  
  TCanvas *dxyAbsMap = new TCanvas("dxyAbsMap","dxyAbsMap",1200,500*nFiles_);
  arrangeCanvas2D(dxyAbsMap,dxyMeanMap,dxyWidthMap,nFiles_,LegLabels,theLeg);
  dxyAbsMap->SaveAs("dxyAbsMap.pdf");
  dxyAbsMap->SaveAs("dxyAbsMap.png");

  TCanvas *dzAbsMap = new TCanvas("dzAbsMap","dzAbsMap",1200,500*nFiles_);
  arrangeCanvas2D(dzAbsMap,dzMeanMap,dzWidthMap,nFiles_,LegLabels,theLeg);
  dzAbsMap->SaveAs("dzAbsMap.pdf");
  dzAbsMap->SaveAs("dzAbsMap.png");

  TCanvas *dxyNormMap = new TCanvas("dxyNormMap","dxyNormMap",1200,500*nFiles_);
  arrangeCanvas2D(dxyNormMap,dxyNormMeanMap,dxyNormWidthMap,nFiles_,LegLabels,theLeg);
  dxyNormMap->SaveAs("dxyNormMap.pdf");
  dxyNormMap->SaveAs("dxyNormMap.png");

  TCanvas *dzNormMap = new TCanvas("dzNormMap","dzNormMap",1200,500*nFiles_);
  arrangeCanvas2D(dzNormMap,dzNormMeanMap,dzNormWidthMap,nFiles_,LegLabels,theLeg);
  dzNormMap->SaveAs("dzNormMap.pdf");
  dzNormMap->SaveAs("dzNormMap.png");
...
//*************************************************************
{
  ...
  for(Int_t i=0; i<nFiles; i++){
    pt[i] = new TPaveText(0.01,0.95,0.89,0.97,"NDC");
  ...
  for(Int_t i=0; i<nFiles; i++){
    ...
    canv->cd(2*i+1)->SetBottomMargin(0.12);
    canv->cd(2*i+1)->SetLeftMargin(0.13);
    canv->cd(2*i+1)->SetRightMargin(0.19);
    canv->cd(2*i+1)->SetTopMargin(0.06);  
...
//*************************************************************
{
  ...
  std::pair<Double_t,Double_t> dummyM;
  std::pair<Double_t,Double_t> dummyW;

  dummyM = std::make_pair(9999.,9999.);
  dummyW = std::make_pair(9999.,9999.);

  std::pair<std::pair<Double_t,Double_t>, std::pair<Double_t,Double_t>  > dummy;
  
  dummy = std::make_pair(dummyM,dummyW);
