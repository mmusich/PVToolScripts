...
void MultiRunPVResolution(TString namesandlabels="",bool lumi_axis_format=false,bool time_axis_format=false,bool useRMS=true);
...
//
///////////////////////////////////
void MultiRunPVResolution(TString namesandlabels,bool lumi_axis_format,bool time_axis_format,bool useRMS){
  ...
  TGraph *g_xResol200_vs_run[nDirs_];
  TGraph *g_yResol200_vs_run[nDirs_];
  TGraph *g_zResol200_vs_run[nDirs_];

  TGraph *g_xResol400_vs_run[nDirs_];
  TGraph *g_yResol400_vs_run[nDirs_];
  TGraph *g_zResol400_vs_run[nDirs_];
  ...
  for(Int_t j=0; j < nDirs_; j++) {
    ...
    g_xResol200_vs_run[j]->GetYaxis()->SetRangeUser(-20.,50.);
    beautify(g_xResol200_vs_run[j]);
 
    my_lego->AddEntry(g_xResol200_vs_run[j],LegLabels[j],"PL");

    if(j==0){
      g_xResol200_vs_run[j]->Draw("AP");
    } else {
      g_xResol200_vs_run[j]->Draw("Psame");
    }
    ...
    g_yResol200_vs_run[j]->GetYaxis()->SetRangeUser(-20.,50.);
    beautify(g_yResol200_vs_run[j]);
 
    if(j==0){
      g_yResol200_vs_run[j]->Draw("AP");
    } else {
      g_yResol200_vs_run[j]->Draw("Psame");
    }
    ...
    g_zResol200_vs_run[j]->GetYaxis()->SetRangeUser(-20.,50.);
    beautify(g_zResol200_vs_run[j]);
 
    if(j==0){
      g_zResol200_vs_run[j]->Draw("AP");
    } else {
      g_zResol200_vs_run[j]->Draw("Psame");
    }
    ...
    g_xResol400_vs_run[j]->GetYaxis()->SetRangeUser(-20.,50.);
    beautify(g_xResol400_vs_run[j]);
 
    if(j==0){
      g_xResol400_vs_run[j]->Draw("AP");
    } else {
      g_xResol400_vs_run[j]->Draw("Psame");
    }
    ...
    g_yResol400_vs_run[j]->GetYaxis()->SetRangeUser(-20.,50.);
    beautify(g_yResol400_vs_run[j]);
 
    if(j==0){
      g_yResol400_vs_run[j]->Draw("AP");
    } else {
      g_yResol400_vs_run[j]->Draw("Psame");
    }
    ...
    g_zResol400_vs_run[j]->GetYaxis()->SetRangeUser(-20.,50.);
    beautify(g_zResol400_vs_run[j]);
 
    if(j==0){
      g_zResol400_vs_run[j]->Draw("AP");
    } else {
      g_zResol400_vs_run[j]->Draw("Psame");
    }
...
/*--------------------------------------------------------------------*/
{
  ...
  for(unsigned int n=first; n<last;n++){
    ...
    TH1F* xPVResolVsPT[nDirs_];  
    TH1F* yPVResolVsPT[nDirs_]; 
    TH1F* zPVResolVsPT[nDirs_];   
    ...
    arrangeOutCanvas(VertexResolutionsVsPt,xPVResolVsPT,yPVResolVsPT,zPVResolVsPT,nDirs_,LegLabels,intersection.at(n));
