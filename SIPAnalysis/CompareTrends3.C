...
void CompareTrends3(TString file1, TString file2,TString file3){
  ...
  TString label1 = file1.ReplaceAll("IOV_ByLumi","").ReplaceAll(".root","");
  TString label2 = file2.ReplaceAll("IOV_ByLumi","").ReplaceAll(".root","");
  TString label3 = file3.ReplaceAll("IOV_ByLumi","").ReplaceAll(".root","");
  ...
  for(int iGraph=0;iGraph<5;iGraph++){
    ...
    c[iGraph] = (TGraph*)f3->Get(listOfGraphs[iGraph]);
    c[iGraph]->SetName(Form("c_%s",listOfGraphs[iGraph].Data()));
    
    c1[iGraph]->cd();

    a[iGraph]->SetLineColor(kRed);
    b[iGraph]->SetLineColor(kBlack);
    c[iGraph]->SetLineColor(kMagenta);
   
    a[iGraph]->SetFillColor(kRed);
    b[iGraph]->SetFillColor(kBlack);
    c[iGraph]->SetFillColor(kMagenta);

    a[iGraph]->SetMarkerColor(kRed);
    b[iGraph]->SetMarkerColor(kBlack);
    c[iGraph]->SetMarkerColor(kMagenta);

    a[iGraph]->SetMarkerStyle(20);
    b[iGraph]->SetMarkerStyle(22);
    c[iGraph]->SetMarkerStyle(21);

    a[iGraph]->SetMarkerSize(1);
    b[iGraph]->SetMarkerSize(1);
    c[iGraph]->SetMarkerSize(1);

    a[iGraph]->SetLineStyle(9);
    b[iGraph]->SetLineStyle(9);
    c[iGraph]->SetLineStyle(9);

    a[iGraph]->SetLineWidth(2);
    b[iGraph]->SetLineWidth(2);
    c[iGraph]->SetLineWidth(2);
    ...
    lego->AddEntry(c[iGraph],label3,"PL");

    TObjArray *array = new TObjArray(3); 
    array->Add(a[iGraph]);
    array->Add(b[iGraph]);
    array->Add(c[iGraph]);
    ...
    c[iGraph]->GetYaxis()->SetRangeUser(extrema.first,extrema.second);
    ...
    c[iGraph]->GetXaxis()->SetNdivisions(305);
