...
foreach i (`seq $#myDirs`)
	...
	if ($inputfile =~ *"root"*) then
	    ...
	    echo $myLFN
	    root -q -b checkLumi.C\(\"$myLFN\"\)  > numevents.out
	    ...
	    printf '%-5s %-5s %-20s %-20s %-20s %-20s\n' $COUNTbyFolder $inputfile $numevents $numTracks $lumi >> report_$myDirs[$i].txt
	endif
    end
end
