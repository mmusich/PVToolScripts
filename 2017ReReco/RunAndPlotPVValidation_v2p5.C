...
void RunAndPlotPVValidation_v2p5(TString namesandlabels,bool lumi_axis_format,bool time_axis_format,bool useRMS){
  ...
  if(lumi_axis_format && time_axis_format){
    std::cout<<"##########################################################################################"<<std::endl;
    std::cout<<"msg-i: RunAndPlotPVValidation_v2p5(): you're requesting both summary vs lumi and vs time, "<< std::endl;
  ...
  for(Int_t j=0; j < nDirs_; j++) {
    ...
    cout<<"RunAndPlotPVValidation_v2p5(): label["<<j<<"]"<<LegLabels[j]<<endl;
