...
CMS_lumi.lumi_sqrtS = "pp collisions, #sqrt{s} = 13 TeV" # used with iPeriod = 0, e.g. for simulation-only plots (default is an empty string)
...
############################
    ...
    #if(len(h1array)>0):
    #print "hist: " ,h1array[0].GetName(), " Minimum: ", the_min, " Maximum: ", the_max
...
######################################################
    ...
    #hist.GetYaxis().SetRangeUser(the_extrema[0]*1.4,the_extrema[1]*1.4)

    if("prof_" not in hist.GetName()):
        if(the_extrema[0]>0):
            hist.GetYaxis().SetRangeUser(0.,the_extrema[1]*1.4)
        else:
            hist.GetYaxis().SetRangeUser(the_extrema[0]*1.4,the_extrema[1]*1.4)
    else:
        hist.GetYaxis().SetRangeUser(the_extrema[0]*1.4,the_extrema[1]*1.4)
...
######################################################
    if(isRatio):
        pt = ROOT.TPaveText(0.67,0.835,0.69,0.895,"NDC")
    else:
        pt = ROOT.TPaveText(0.6,0.835,0.62,0.895,"NDC")
    pt.SetFillColor(10)
    pt.SetTextColor(1)
    pt.SetTextFont(61)
    pt.SetTextAlign(12)
    ...
    if(isRatio):
        pt2 = ROOT.TPaveText(0.72,0.845,0.92,0.895,"NDC")
    else:
        pt2 = ROOT.TPaveText(0.65,0.845,0.75,0.895,"NDC")
    pt2.SetFillColor(10)
    pt2.SetTextColor(1)
    pt2.SetTextFont(52)
    pt2.SetTextAlign(11)
    ...
    pt3.SetTextFont(42)
    # pt2.SetTextAlign(11)
    text3 = pt3.AddText("pp #sqrt{s}=13 TeV 2015")
        ...
        for k in dirList:
            ...
            if (obj1.IsA().InheritsFrom("TH1")):#and "h2_" not in obj1.GetName()):
                ...
                for clone in clones:  
                    ...
                    if("prof_" in obj1.GetName() and obj1.IsA().InheritsFrom("TProfile")):
                ...
                for h1 in obj_array: 
                    ...
                    if( (h1.GetMinimum()>0.)):
                        h1.GetYaxis().SetRangeUser(the_extrema[0],the_extrema[1]+ROOT.TMath.Power(10,dr))
                        #h1.GetYaxis().UnZoom()
                    else :
                        h1.GetYaxis().SetRangeUser(0.01,ROOT.TMath.Power(10,1.3*ROOT.TMath.Log10(10*the_extrema[1])))
...
#############################################
    ...
    outputFolder =  "just_testingICHEP"
