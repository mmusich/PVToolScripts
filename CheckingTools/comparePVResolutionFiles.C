...
//*****************************************************//
{
  ...
  while (TKey* key = (TKey*)nextkey()) {
    ...
    if ( obj->IsA()->InheritsFrom("TDirectory") ){
      f1->cd(("PrimaryVertexResolution/"+name).c_str());
      TIter nextkey(gDirectory->GetListOfKeys());                                                                             
      while (key = (TKey*)nextkey()) { 
	obj = key->ReadObj();                                                                                         
	if (obj->IsA()->InheritsFrom("TH1")) {                                                                                
	  TH1* h = (TH1*)obj;                                                                              
	  TString fullpath = "PrimaryVertexResolution/"+name+"/"+h->GetName();
      ...
      obj = key->ReadObj();                                                                                         
      TH1* h = (TH1*)obj;                                                                              
      TString fullpath = "PrimaryVertexResolution/"+(TString)h->GetName();
      //std::cout << "j: " << j << " "<< h->GetName() <<" "<< fullpath << std::endl;
      ++j;
      if (obj->IsA()->InheritsFrom("TH2")) continue;
      TCanvas *c1 = new TCanvas(h->GetName(),h->GetName(),800,600);
